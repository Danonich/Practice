...
class Ball {
    ...
    draw(frame) {
        for (let oy = -this.radius; oy < this.radius; oy++) {
            for (let ox = -this.radius; ox < this.radius; ox++) {
                const oz = Math.sqrt(Math.pow(ox, 2) + Math.pow(oy, 2));

                if (oz < this.radius) {
                    frame.setPixel(this.pos.x + ox, this.pos.y + oy, this.color, frame.pixels);
                }
            }
        }
    }
